...
class Grafico {
   ...
   * @param {number} x Abcissa do ponto a testar se está contido no <em>gráfico</em>
...
class Circulo extends Grafico {
   ...
   * @param {number} x Abcissa do ponto a testar se está contido neste <em>círculo</em>
...
class Rectangulo extends Grafico {
   ...
   * @param {number} x Abcissa do ponto a testar se está contido neste <em>rectângulo</em>
...
class Poligono extends Grafico {
  ...
  */
  ...
  /**
    ...
    * @param {number} x Abcissa do ponto a testar se está contido neste <em>polígono</em>
 ...
 * @property {number} x Abcissa para posicionar o <em>texto</em> no <em>canvas</em>
 * @property {number} y Ordenada para posicionar o <em>texto</em> no <em>canvas</em>
 * @property {string} preenchimento Cor do preenchimento <em>texto</em> &mdash; se a cor for especificada como <code>empty</code>, o <em>texto</em> não é preenchido
 * @property {string} contorno Cor do contorno do <em>texto</em> &mdash; se a cor for especificada como <code>empty</code>, o <em>texto</em> o contorno não é desenhado
...
class Texto extends Grafico {
   ...
   * @param {number} x Abcissa para posicionar o <em>texto</em> no <em>canvas</em>
   * @param {number} y Ordenada para posicionar o <em>texto</em> no <em>canvas</em>
   * @param {string} preenchimento="black" Cor do preenchimento <em>texto</em>
   ...
   * @param {number} x Abcissa do ponto a testar se está contido neste <em>texto</em>
 ...
 * @property {number} x Abcissa para posicionar a <em>imagem</em> no <em>canvas</em>
...
class Imagem extends Grafico {
   ...
   * @param {number} x Abcissa para posicionar a <em>imagem</em> no <em>canvas</em>
   ...
   * @param {number} x Abcissa do ponto a testar se está contido nesta <em>imagem</em>
 ...
 * @class
 * @extends Imagem
 * @classdesc A classe <code>ImagemAnimada</code> é uma subclasse de <code>Grafico</code>, servindo para representar <em>imagens animadas</em> (ou <em>sprites</em>). Num nível básico, uma <em>imagem</em> é definida por um <em>ponto</em>, correspondente, regra geral, ao seu canto superior esquerdo e pela própria imagem.
 * @property {number} x Abcissa para posicionar o <em>sprite</em> no <em>canvas</em>
 ...
 * @property {boolean} seleccionado=false Indicação de que o <em>sprite</em> se encontra seleccionado &mdash; pode ser usado, por exemplo, para indicar que foi seleccionado com o cursor do rato
  */class ImagemAnimada extends Imagem {
   ...
   * @param {number} x Abcissa para posicionar o <em>sprite</em> no <em>canvas</em>
  ...
  /**
   * Este método desenha uma <em>imagem animada</em> no <em>canvas</em>, usando como referência de posicionamento o <em>ponto</em> correspondente ao canto superior esquerdo dessa <em>imagem animada</em>.
    ...
    if (this.visivel) {
      ...
      if ((this.animada) || (this.indice > 0)) {
        if (this.contador % this.iteracoes == 0) {
          this.indice++;
          if (this.indice >= this.fotogramas) {
            this.indice = 0;
          }
        }
        this.contador++;
...
/**
 * Esta função desenha um <em>gráfico</em> ou percorre um <em>array</em> de <em>gráficos</em> (instâncias ou objectos de subclasses da classe <code>Grafico</code>) e desenha-os no <em>canvas</em> especificado. O <em>array</em> pode conter também <em>arrays</em> em qualquer das suas posições, funcionando, esta função, de forma recursiva.
 ...
 * @property {HTMLElement} tela O <em>elemento</em> HTML a ser representado
 */
class Tela {
  /**
   * Construtor da classe <code>Tela</code>.
   * @param {HTMLElement} tela O <em>elemento</em> HTML a ser representado
   */
  constructor(tela) {
    this.tela = tela;
  }
  ...
  /**
* Elemento original usado na criação de uma instância da classe <code>Tela</code>, para que possa ser manipulado directamente &mdash; por exemplo, quando é necessário adicionar eventos ao elemento
   * @type {HTMLElement}
   */
  get elemento() {
    return this.tela;
 ...
 * @property {HTMLElement} media O <em>elemento</em> HTML a ser representado
 */
class Media {
  /**
   ...
   * @param {HTMLElement} media O <em>elemento</em> HTML a ser representado
   */
  constructor(media) {
    this.media = media;
  ...
  /**
* Elemento original usado na criação de uma instância de uma das subclasses de <code>Media</code>, para que possa ser manipulado directamente &mdash; por exemplo, quando é necessário adicionar eventos ao elemento
   * @type {HTMLElement}
   */
  get elemento() {
    return this.media;
 ...
 * @property {HTMLElement} som O <em>elemento</em> HTML do tipo <code>som</code> a ser representado
 */
class Som extends Media {
  /**
   * Construtor da classe <code>Som</code>
   * @param {HTMLElement} som  O <em>elemento</em> HTML do tipo <code>som</code> a ser representado
   */
  constructor(som) {
    super(som);
  }
 ...
 * @property {HTMLElement} filme O <em>elemento</em> HTML do tipo <code>video</code> a ser representado
 */
class Filme extends Media {
  /**
   * Construtor da classe <code>Filme</code>
   * @param {HTMLElement} filme O <em>elemento</em> HTML do tipo <code>video</code> a ser representado
   */
  constructor(filme) {
    super(filme);
  }
